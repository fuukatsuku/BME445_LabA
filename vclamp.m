...
% YOUR EXAMPLE CODE:
if (ionflag == 8) %1mg of TTX
     G_NA = 0.0;  %mS/cm^2
elseif(ionflag == 7) %.75mg of TTX 
    G_NA = 120- 120 *.75; %Not Mathematically or scientifically correct in the slightest
elseif (ionflag == 6) %.50mg of TTX
    G_NA = 120- 120 *.50;
elseif (ionflag == 5)%.25mg of TTX
    G_NA = 120- 120 *.25;
elseif (ionflag == 4)
    G_NA = 120- 120 *.01;
end
...
elseif displayflag == 3
    ...
    plot(t,g_Na,'b-');
    axis([0 T_MAX ymin ymax]);
    title('Sodium Conductance vs Time Plot');
    xlabel('time (mSec)');
    ylabel('g_Na ');
    
elseif displayflag == 4
    
    plot(t,G_K,'b-');
    axis([0 T_MAX ymin ymax]);
    title('Potassium Current vs Time Plot');
    xlabel('time (mSec)');
    ylabel('g_Na ');
end
...
eval(sprintf('save im_%s Im meta', ['part2b']));
% save ...
